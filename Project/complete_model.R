...
ui <- fluidPage(theme = shinytheme("sandstone"),
  img(src = "sig.jpg", height = 72, width = 72),
  tabsetPanel(
  #tabpanel1 ----
  tabPanel(
    " SIG Customer-ProductPaired INfo",
    # Input: Select a file ----
    ...
    selectInput("customerproductpaired", "Customer Product",
                choices = c(Thailand = "Thailand",
                            Indonesia = "Indonesia")),
    # Horizontal line ----
    tags$hr(),
  ...
  tabPanel("QuickModel",
           ...
           # Input: customerproductpaired ----
           selectInput("customerproductpaired1", "Customer Product",
                       choices = c(Thailand = "Thailand",
                                   Indonesia = "Indonesia")),
           plotOutput("MAplot"),
           # Output: QuicKModel ----
  ...
  #tabpanel4 ----
  tabPanel("Team5 Model",
           # Output: downloadData ----
           downloadButton("downloadData1", "Download"),
           # Input: customerproductpaired ----
           selectInput("customerproductpaired2", "Customer Product",
                       choices = c(Thailand = "Thailand",
                                   Indonesia = "Indonesia")),
           plotOutput("Team5plot"),
...
server <- function(input, output,session) {
    ...
    tryCatch(
      {
        ...
        updateSelectInput(session, "customerproductpaired2",
                          choices = x,
                          selected = x
        )
      },
    ...
    tryCatch(
      {
        ...
        Regular<<-c('Active Customer',colnames(sig_pred_actual))
        Past<<-c('Nonactive Customer',colnames(sig_drop))
        NewC<<-c('New Customer',colnames(sig_drop))
    ...
    tryCatch(
      {
        ...
        f2019_sheet<<-matrix(c('customer','model name',as.numeric(1:12)))
        RMSE_value = 0
        
        for (i in 1:length(sig_pred_actual)){
          nam <- colnames(sig_pred_actual)[i]
          #define object
          package.ts <-ts(sig_pred_actual[,i], start = c(2009,1), freq =12)
          ...
          ##model select_accuracy
          modelsF<-list(validSnaiveF,validhwinF,validMAF,validlmF)
          modelsE<-list(validSnaiveE,validhwinE,validMAE,validlmE)
          modelsNames<-c('Snaive','hwin','MA','lm')
          ...
          if (modelname == "Snaive"){
            Snaive.fore <- snaive(package.ts, h=12)
            f <- Snaive.fore$mean
          }else if(modelname == "hwin"){
            hwin <- ets(package.ts)
            f  <-forecast(hwin,h=12)$mean
          }else if(modelname == "MA"){
            package.ma=rollmean(package.ts,k=12,align='right')
            validMAF<<-ts(tail(package.ma,12),start = c(2018,1),frequency = 12)
            f<-validMAF
          }else if(modelname == "lm"){
            lmM<-tslm(package.ts~trend+season)
            f<-forecast(lmM,h=12)$mean
          }
          ...
          forecast_sheet <- c(nam,modelname,f)
          bestF_sheet <<- cbind(bestF_sheet,as.data.frame(bestF))
          f2019_sheet <<- cbind(f2019_sheet,as.data.frame(forecast_sheet))
          RMSE_value <<- bestAccuracy + RMSE_value
        }
        for (i in 1:length(sig_drop)){
          nam <- colnames(sig_drop)[i]
          package.ts <-ts(sig_drop[,i], start = c(2009,1), freq =12)
          valid.ts <- subset(package.ts, start = length(package.ts) - 12+1)
          f_zero <- ts(rep(0, 12),start = c(2018,1),frequency = 12)
          rmse_value <- accuracy(valid.ts,f_zero)[2]
          bestF<-c(nam,"Zero",rmse_value,f_zero)
          bestF_sheet <<- cbind(bestF_sheet,as.data.frame(bestF))
        }
        return(f2019_sheet)
      },
  ...
  output$downloadData1 <- downloadHandler(
    ...
    content = function(file) {
      write.xlsx(data_w_nams, file, sheetName="data", row.names=FALSE)
      write.xlsx(bestF_sheet, file, sheetName="bestF_sheet", append=TRUE, row.names=FALSE)
      write.xlsx(f2019_sheet, file, sheetName="2019_forecast", append=TRUE, row.names=FALSE)
    }
  )
  
  # renderPlot: Team5PLOot ----
  output$Team5plot <- renderPlot({
    ...
    req(input$file1)
    req(input$customerproductpaired2)
    nam<-input$customerproductpaired2
    #######trycatch########
    tryCatch(
      {
        df1 <<- read.csv(input$file1$datapath,
