...
ui<-dashboardPage(
  dashboardHeader(title='SIG Project'),
  dashboardSidebar(
    sidebarMenu(
      menuItem("Actual", tabName = "dashboard", icon = icon("dashboard")),
      menuItem("QuickForecast", tabName = "QuickForecast", icon = icon("th")),
      menuItem("Team5Forecast", tabName = "Team5Forecast", icon = icon("th"))
    )
  ),
  dashboardBody(
    tabItems(
...
server <- function(input, output,session) {
    ...
    tryCatch(
      {
        ...
        ma_model(package.ts,valid.ts)
        actual_plot<-autoplot(package.ts, series='Data',alpha=0.2) +
          forecast::autolayer(validMAF,series='QuickForecast',linetype='dashed')+
  ...
  output$downloadData1 <- downloadHandler(
    ...
    content = function(file) {
      best_model_datasets<-list('data'=data_w_nams,'bestF_sheet'=t(bestF_sheet),'bestE_sheet'=t(bestE_sheet))
